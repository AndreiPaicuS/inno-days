...
export default function Resources() {
  ...
  async function handleResource() {
    if (isEditing) {
      ...
      setResources(
        resources.map((res) =>
          res.id === editResourceId ? { ...res, ...resource } : res
        )
      );
      setIsEditing(false);
  ...
  return (
    <div className="p-4 bg-gray-100 min-h-screen">
      ...
      <form
        className="mb-6 grid grid-cols-2 gap-4 bg-white p-6 rounded-lg shadow-md"
      ...
      >
        ...
        <select
          ...
          onChange={(e) =>
            setResource({ ...resource, skillId: e.target.value })
          }
          className="mb-2 px-4 py-2 border rounded w-full"
        >
        ...
        <select
          ...
          onChange={(e) =>
            setResource({ ...resource, levelOfExpertise: e.target.value })
          }
          className="mb-2 px-4 py-2 border rounded w-full"
        >
        ...
        <input
          ...
          onChange={(e) =>
            setResource({ ...resource, yearsAtNatterbox: e.target.value })
          }
          className="mb-2 px-4 py-2 border rounded w-full"
        ...
        <input
          ...
          onChange={(e) =>
            setResource({ ...resource, project: e.target.value })
          }
          className="mb-2 px-4 py-2 border rounded w-full"
        ...
        <input
          ...
          onChange={(e) =>
            setResource({ ...resource, endDate: e.target.value })
          }
        ...
        <tbody>
          {resources.map((resource) => (
            <tr key={resource.id} className="hover:bg-gray-100">
              ...
              <td className="py-2 px-4 border-b text-left">
                {
                  skills.find((skill) => skill.id === resource.skillId)
                    ?.skillName
                }
              </td>
              <td className="py-2 px-4 border-b text-left">
                {resource.levelOfExpertise}
              </td>
              <td className="py-2 px-4 border-b text-left">
                {resource.yearsAtNatterbox}
              </td>
              <td className="py-2 px-4 border-b text-left">
                {resource.project}
              </td>
